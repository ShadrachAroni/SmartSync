...
> - Sensor power: 5 V (PIR, HC-SR04, DS3231, LCD, buzzer if required)  
...
| Buck OUT + (5 V) | Buck converter + | ESP32 VIN, PIR VCC, HC-SR04 VCC, DS3231 VCC, LCD VCC, buzzer VCC | Ensure ≥1 A capability. |
...
### 20×4 Character LCD Display (I²C)

- VCC → 5 V rail  
- GND → Common ground  
- SDA → `GPIO21` (`RTC_SDA_PIN`) - **Shared I²C bus with RTC**  
- SCL → `GPIO22` (`RTC_SCL_PIN`) - **Shared I²C bus with RTC**  
- I²C Address: `0x27` (default, may vary; check with I²C scanner if needed)  
- **Note:** LCD shares the same I²C bus as the DS3231 RTC. Both devices use different I²C addresses, so they can coexist on the same bus. The LCD I²C backpack typically includes pull-up resistors, but verify they are 4.7 kΩ to 3.3 V for ESP32 compatibility.
